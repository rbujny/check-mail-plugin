...
/**
 * Finds the Onet Mail message toolbar in the standard view.
 */
function findOnetToolbar(): HTMLElement | null {
    const toolbar =
        document.querySelector<HTMLElement>('[data-test="mail-toolbar"]') ||
        document.querySelector<HTMLElement>('[role="toolbar"]') ||
        document.querySelector<HTMLElement>('.mail-toolbar');
    ...
    for (const btn of buttons) {
        ...
        if (text === 'odpowiedz' || text === 'przekaż' || testId.includes('reply') || testId.includes('forward')) {
...
/**
 * Inject the shield icon into the Onet Mail standard view toolbar.
 */
export function injectOnetStandardIcon(): void {
    ...
    try {
        const toolbar = findOnetToolbar();
        if (toolbar) {
            const button = createStandardShieldButton();
            toolbar.appendChild(button);
        } else {
            console.warn('[CheckMailPlugin][Onet] Standard view: toolbar not found for injection');
        }
    } catch (error) {
...
// ─── Raw View ("Pokaż źródło wiadomości") ───────────────────────────
...
/**
 * Inject a floating extraction button in the Onet raw source view.
 */
